...
def zip_file(src_dir, zip_name="", parent_dir_name=""):
    ...
    if "w" == mode and parent_dir_name == "":
        ...
        # yasuo = f"7z -r  {os.path.abspath(zip_name)} *"
        # status, message = execute_cmd(yasuo)
        # print( status, message)
        # 
        # cmd2 = ['7z', 'a', f"{os.path.abspath(zip_name)}", "*"]
        # # cmd2 = ['7z', 'a', f"{zip_name}", ".\*"]
        ...
        # print(output)
        # message = ""
        # message222 = output.decode('utf-8').split('\n')
        # # if message is not None:
        # #     message = message222[0]
        # print(message222,"error: ",err) 
        # if len(message222[0]) >1:
        #     status = 0
        # else:
        #       return -1, "error",
        #
