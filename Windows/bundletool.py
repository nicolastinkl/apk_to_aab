...
def task(task_name, fun, *args, **kwargs):
    ...
    end_time = time.time()
    print_log(f"###耗时:{round((end_time - start_time), 3)}s {task_name} status:{status} msg:{msg}")
    if status != 0:
...
class Bundletool:
    ...
    def check_system(self, apk_path, out_aab_path):
        ...
        print("keystore......",status, msg)
        if status != 0:
           return -999, "keystore 密碼错误"
    ...
    def run(self, apk_path, out_aab_path, pad_reg=""):
        ...
        try:
            ...
            print_log(f"*********UPDATE NOTE*********")
            print_log(f"apk_package_name: {self.apk_package_name}")
            print_log(f"version_code: {self.version_code}")
            print_log(f"versionName: {self.version_name}") 
